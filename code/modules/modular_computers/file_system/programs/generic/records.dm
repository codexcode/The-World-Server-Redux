...
/datum/nano_module/records/ui_interact(mob/user, ui_key = "main", datum/nanoui/ui = null, force_open = 1, state = default_state)
	var/list/data = list()
	if(program)
	...
	if(active_record)
		...
		for(var/datum/computer_file/report/crew_record/R in all_crew_records)
...
/datum/nano_module/records/Topic(href, href_list)
	...
	if(href_list["new_record"])
		...
		all_crew_records.Add(active_record)
		return 1
