...
var/list/default_physical_status = list("Active")
var/list/security_statuses = list("None", "Released", "Parolled", "Incarcerated", "Arrest")
var/list/default_security_status = list("None")
var/list/arrest_security_status = list("Arrest")
...
//	set_formal_name(formal_name)
	...
	if(H)
		gender_term = H.gender
	set_sex(gender_term)
	set_age(H ? H.age : 30)
	set_status(default_physical_status)
...
//	set_rank(H ? (H.char_rank && H.char_rank.name) : "None")
	...
	// Security record
	set_criminalStatus(default_security_status)
...
/proc/department_crew_manifest(var/list/filter_positions, var/blacklist = FALSE)
	...
	for(var/datum/computer_file/report/crew_record/CR in all_crew_records)
		var/rank = CR.get_job()
		if(blacklist)
...
FIELD_LIST_EDIT("Status", status, physical_statuses, null, access_medical)
...
// MEDICAL RECORDS
FIELD_LIST("Blood Type", bloodtype, blood_types, access_medical, access_medical)
...
// SECURITY RECORDS
FIELD_LIST("Criminal Status", criminalStatus, security_statuses, access_security, access_security)
